...
export class UploadComponent implements OnInit, OnChanges {
  ...
  uploadFile = (files:any) => {
    ...
    this.http.post(environment.apiURL+'/upload?documentType=property', formData, {reportProgress: true, observe: 'events'})
      .subscribe({
        next: (event) => {
  ...
  viewFile(): void {
    console.log('UploadComponent.viewFile called', {
      uploadedFilePath: this.uploadedFilePath,
      uploadedFileName: this.uploadedFileName
    });
    
    if (this.uploadedFilePath) {
      console.log('Opening dialog with file:', {
      ...
      console.warn('UploadComponent.viewFile: No file path available');
    }
  }
