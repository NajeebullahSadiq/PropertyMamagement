...
export class VehicleNationalidUploadComponent implements OnInit {
  ...
  uploadFile = (files:any) => {
    console.log('Vehicle National ID Upload: Starting upload process', files);
    if (files.length === 0) {
      console.log('Vehicle National ID Upload: No files selected');
      return;
    }

    let fileToUpload = <File>files[0];
    console.log('Vehicle National ID Upload: File selected', fileToUpload.name, fileToUpload.size);
    ...
    console.log('Vehicle National ID Upload: Calling API', uploadUrl);
    
    this.http.post(uploadUrl, formData, {reportProgress: true, observe: 'events'})
      .subscribe({
        next: (event) => {
        console.log('Vehicle National ID Upload: API Response', event);
