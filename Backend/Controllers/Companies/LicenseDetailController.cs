    ...
    public class LicenseDetailController : ControllerBase
    {
        ...
        public async Task<ActionResult<int>> SaveProperty([FromBody] LicenseDetailData request)
        {
            ...
            try
            {
                ...
                // Admin can provide a custom license number; otherwise it will be auto-generated when IsComplete becomes true
                string? licenseNumber = null;
                if (_provinceFilter.IsAdministrator() && !string.IsNullOrWhiteSpace(request.LicenseNumber))
                {
                    // Admin provided a custom license number - use it
                    licenseNumber = request.LicenseNumber;
                }

                var property = new LicenseDetail
                {
                    LicenseNumber = licenseNumber, // Admin custom or null for auto-generation
        ...
        public async Task<IActionResult> UpdateCompanyDetails(int id, [FromBody] LicenseDetailData request)
        {
            try
            {
                ...
                var originalLicenseNumber = existingProperty.LicenseNumber; // Preserve existing license number

                // Admin can update the license number; otherwise preserve the existing one
                // Auto-generation happens when IsComplete becomes true via CompanyService
                if (_provinceFilter.IsAdministrator() && !string.IsNullOrWhiteSpace(request.LicenseNumber))
                {
                    existingProperty.LicenseNumber = request.LicenseNumber; // Admin custom license number
                }
                else
                {
