...
{
    /// <summary>
    /// Migration to restrict Vehicle Buyer Role Types to 3 approved options:
    ...
    /// 
    /// Legacy role types are mapped to the closest approved option for backward compatibility.
    /// </summary>
    public partial class RestrictVehicleBuyerRoleTypes : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Map deprecated buyer role types to approved options
            // Mapping strategy:
            // - 'Buyer in a revocable sale' -> 'Buyer'
            // - 'Buyers in a revocable sale' -> 'Buyers'
            // - 'Lessee' -> 'Buyer'
            // - 'Lessees' -> 'Buyers'
            // - 'Agent for buyer in a revocable sale' -> 'Purchase Agent'
            // - 'Agent for lessee' -> 'Purchase Agent'
            // - Any other invalid value -> 'Buyer'
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            // Restore original values from migration log (reversible migration)
            migrationBuilder.Sql(@"
                UPDATE ""VehiclesBuyerDetails"" vbd
                SET ""RoleType"" = log.""OldRoleType""
                FROM ""VehiclesBuyerRoleTypeMigrationLog"" log
                WHERE vbd.""Id"" = log.""BuyerId"";
            ");

            // Note: Migration log table is kept for audit purposes
        }
    }
}
